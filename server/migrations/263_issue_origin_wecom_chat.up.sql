...
-- migrations — including the downstream 'external_issue' (migration 117) and
-- upstream 'dingtalk_chat' (migration 259) — or this rebuild would silently
-- drop them. Keep this list in sync with the newest issue_origin_* migration
-- when rebasing.
--
...
ALTER TABLE issue ADD CONSTRAINT issue_origin_type_check
    CHECK (origin_type IN ('autopilot', 'quick_create', 'lark_chat', 'external_issue', 'slack_chat', 'agent_create', 'dingtalk_chat', 'wecom_chat'))
    NOT VALID;
