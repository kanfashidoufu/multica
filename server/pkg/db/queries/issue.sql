...
UPDATE issue SET
    ...
    metadata = $5,
    revision = revision + (
        ROW(title, description, status, priority, metadata)
        IS DISTINCT FROM ROW($1::text, $2::text, $3::text, $4::text, $5::jsonb)
    )::integer,
    last_activity_at = CASE
        WHEN ROW(title, description, status, priority, metadata)
            IS DISTINCT FROM ROW($1::text, $2::text, $3::text, $4::text, $5::jsonb)
        THEN GREATEST(COALESCE(last_activity_at, updated_at), now())
        ELSE last_activity_at
    END,
