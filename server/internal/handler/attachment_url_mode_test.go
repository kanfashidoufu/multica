...
func TestAttachmentToResponse_StableModeDropsSignature(t *testing.T) {
	...
	signed := testHandler.attachmentToResponse(context.Background(), att, attachmentURLModeSigned)
	stable := testHandler.attachmentToResponse(context.Background(), att, attachmentURLModeStable)
...
func TestAttachmentToResponse_SignedModeRotatesButStableDoesNot(t *testing.T) {
	...
	stableA := testHandler.attachmentToResponse(context.Background(), att, attachmentURLModeStable)
	stableB := testHandler.attachmentToResponse(context.Background(), att, attachmentURLModeStable)
...
// TestAttachmentToResponse_StableModeIsNoOpForProxy pins proxy deployments:
// they already return the stable endpoint, so the capability changes nothing.
