...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	// Syndra external issue sync webhook. Authenticated by an operator-provided
	// bearer token and intentionally isolated from the user-session issue API.
	r.Post("/api/webhooks/external-issues", h.ImportExternalIssue)
