...
设置 `OSS_BUCKET` 时优先使用阿里云 OSS；否则设置了 `S3_BUCKET` 就使用 S3；两者都未设置时回落到本地磁盘。
...
| `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET` | 空 | 静态 OSS 凭据 |
| `OSS_SECURITY_TOKEN` | 空 | 可选 STS 临时凭据 token |
| `OSS_PUBLIC_BASE_URL` | 空 | 附件读取使用的 CDN 或自定义域名。不设时返回 `https://<bucket>.oss-<region>.aliyuncs.com/<key>` |
...
飞书多维表格自动化等外部系统需要通过 `POST /api/webhooks/external-issues` 创建 Multica issue 时，配置下面这些变量。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | 空 | 接口要求的共享密钥。请求必须带 `Authorization: Bearer <token>`；不设置时返回 `503 external issue import is not configured` |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_WORKSPACE_ID` | 空 | 请求未传 `workspace_id` 时使用的默认工作区 ID |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_LARK_USER_ID` | 空 | 默认负责人和创建人对应的飞书/Lark `user_id`；该账号必须已绑定目标工作区中的 Multica 用户 |
| `MULTICA_EXTERNAL_ISSUE_LARK_OPENAPI_TIMEOUT` | `30s` | 回查飞书 Base 记录的超时时间，使用 Go duration，例如 `45s` |

飞书多维表格自动化的 HTTP 节点传入 `app_token`、`table_id`、`record_id`。Multica 使用 `LARK_APP_ID` 和 `LARK_APP_SECRET` 回查记录，只导入 `版本类型` 为 `小需求` 的数据，并把 Base 附件上传为 issue 附件。导入的 issue 默认进入 `backlog`，并以 `origin_type=external_issue` 保证幂等。

## Release 发布

| 变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_RELEASE_REPOSITORY` | 当前 CI 仓库 | Release 资产仓库，格式为 `owner/repo`；Web 下载页、CLI 更新和 Desktop 打包都会读取它 |
...
- [GitHub 集成](/github-integration) — 配置集成使用的 GitHub App。
