...
Syndra 通过 `POST /api/webhooks/external-issues` 推送 Bug 或小需求事件时，配置下面这些变量。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | 空 | 同步接口要求的共享密钥。请求必须带 `Authorization: Bearer <token>`；不设置时返回 `503 external issue sync is not configured` |
| `MULTICA_EXTERNAL_BUG_WORKSPACE_ID` | 空 | `sync_type=bug` 固定使用的 Bug 工作区 ID；不接受请求覆盖 |
| `MULTICA_EXTERNAL_REQUIREMENT_WORKSPACE_ID` | 空 | `sync_type=requirement` 固定使用的小需求工作区 ID；不接受请求覆盖，确保与 Bug 工作区隔离 |

现有 Syndra Bug upsert 流程使用 `sync_type=bug`。Bug 同步优先按推送的负责人名称匹配工作区成员；名称缺失或无法唯一匹配时，回退到 Bug 工作区的 owner（工作区创建人）。小需求同步使用 `sync_type=requirement`，由请求中的 `executor_id` 指派 Multica 智能体，并使用该智能体的负责人作为 issue 创建人；旧默认工作区、默认负责人环境变量和飞书多维表格参数不再支持。
