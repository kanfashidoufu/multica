...
Multica 会使用飞书登录应用的 `LARK_APP_ID` 和 `LARK_APP_SECRET` 回查这条多维表格记录，只导入 `版本类型` 为 `小需求` 的记录。issue 内容会包含 `需求名称`、`产品经理`、`备注` 和来源链接。附件会按飞书多维表格附件字段里的 `file_token`、`name`、`size`、`type`、`url`、`tmp_url` 解析后上传为 issue 附件。
