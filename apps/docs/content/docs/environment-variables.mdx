...
Multica uses Alibaba Cloud OSS when `OSS_BUCKET` is set, then S3 when `S3_BUCKET` is set, and otherwise falls back to local disk.
...
Set these when an external automation, such as Feishu Base automation, should create Multica issues through `POST /api/webhooks/external-issues`.

| Variable | Default | Description |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | empty | Shared secret required by the external issue import endpoint. Requests must send `Authorization: Bearer <token>`. When unset, the endpoint returns `503 external issue import is not configured` |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_WORKSPACE_ID` | empty | Default workspace id used when the automation payload omits `workspace_id`. Feishu Base automation usually sends only `app_token`, `table_id`, and `record_id`, so set this for that flow |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_LARK_USER_ID` | empty | Feishu/Lark `user_id` used as the default Multica member assignee and creator. The matching Lark account must already be linked to a Multica user who belongs to the target workspace |
| `MULTICA_EXTERNAL_ISSUE_LARK_OPENAPI_TIMEOUT` | `30s` | Timeout for fetching the Feishu Base record; accepts a Go duration such as `45s` |

For Feishu Base automation, send `app_token`, `table_id`, and `record_id` as HTTP parameters. Multica uses `LARK_APP_ID` and `LARK_APP_SECRET` to fetch the record, imports records whose `版本类型` is `小需求`, and uploads Base attachment fields as issue attachments. Imported issues start in `backlog` and use `origin_type=external_issue` for idempotency.

## Release publishing

| Variable | Default | Description |
|---|---|---|
| `MULTICA_RELEASE_REPOSITORY` | current CI repository | Repository slug (`owner/repo`) used by the web download page, CLI updates, and Desktop packaging for release assets |
...
- [GitHub integration](/github-integration) — configure the GitHub App used by the integration.
