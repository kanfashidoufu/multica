...
Multica uses `LARK_APP_ID` and `LARK_APP_SECRET` from the Lark login app to fetch that one Base record, then imports records whose `版本类型` is `小需求`. Attachments are read from Feishu Base attachment field values (`file_token`, `name`, `size`, `type`, `url`, `tmp_url`) and uploaded as issue attachments.
