...
### Syndra external issue sync

Set these when Syndra sends bug or requirement events through `POST /api/webhooks/external-issues`.

| Variable | Default | Description |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | empty | Shared secret required by the sync endpoint. Requests must send `Authorization: Bearer <token>`. When unset, the endpoint returns `503 external issue sync is not configured` |
| `MULTICA_EXTERNAL_BUG_WORKSPACE_ID` | empty | Dedicated workspace id used by `sync_type=bug`. Requests cannot override it |
| `MULTICA_EXTERNAL_REQUIREMENT_WORKSPACE_ID` | empty | Dedicated workspace id used by `sync_type=requirement`. The request cannot override it, keeping requirement and bug workspaces isolated |

Use `sync_type=bug` for the existing Syndra bug upsert flow. Bug sync resolves the assignee from the pushed name and falls back to the bug workspace owner when the name is missing or cannot uniquely match a member. Requirement sync uses `sync_type=requirement`, assigns the Multica agent from `executor_id`, and records that agent's owner as the issue creator. Former default-workspace/default-assignee variables and Feishu Base parameters are no longer supported.
