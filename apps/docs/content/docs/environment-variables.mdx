...
Multica uses `LARK_APP_ID` and `LARK_APP_SECRET` from the Lark login app to fetch that one Base record, then imports records whose `版本类型` is `小需求`. The issue description includes `需求名称`, `产品经理`, `备注`, and the source URL. Attachments are read from Feishu Base attachment field values (`file_token`, `name`, `size`, `type`, `url`, `tmp_url`) and uploaded as issue attachments.
