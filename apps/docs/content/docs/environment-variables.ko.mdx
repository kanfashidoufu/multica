...
Syndra가 `POST /api/webhooks/external-issues`로 Bug 또는 소규모 요구사항 이벤트를 보낼 때 설정합니다.

| 변수 | 기본값 | 설명 |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | 비어 있음 | 동기화 엔드포인트에 필요한 공유 secret. 요청은 `Authorization: Bearer <token>`을 보내야 합니다. 설정하지 않으면 엔드포인트가 `503 external issue sync is not configured`를 반환합니다 |
| `MULTICA_EXTERNAL_BUG_WORKSPACE_ID` | 비어 있음 | `sync_type=bug` 전용 workspace id. 요청으로 재정의할 수 없습니다 |
| `MULTICA_EXTERNAL_REQUIREMENT_WORKSPACE_ID` | 비어 있음 | `sync_type=requirement` 전용 workspace id. 요청으로 재정의할 수 없으며 요구사항과 Bug workspace를 분리합니다 |

기존 Syndra Bug upsert 흐름에는 `sync_type=bug`를 사용합니다. Bug 동기화는 전송된 담당자 이름으로 workspace member를 찾고, 이름이 없거나 한 명으로 일치하지 않으면 Bug workspace의 owner(workspace 생성자)로 대체합니다. 소규모 요구사항 동기화는 `sync_type=requirement`를 사용하고 `executor_id`의 Multica Agent를 할당하며 해당 Agent의 owner를 issue creator로 기록합니다. 기존 기본 workspace/assignee 변수와 Feishu Base 파라미터는 더 이상 지원하지 않습니다.
