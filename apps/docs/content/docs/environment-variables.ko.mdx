...
`OSS_BUCKET`이 설정되어 있으면 Alibaba Cloud OSS를 우선 사용하고, 다음으로 `S3_BUCKET`이 설정되어 있으면 S3를 사용합니다. 둘 다 없으면 로컬 디스크를 사용합니다.

### Alibaba Cloud OSS

| 변수 | 기본값 | 설명 |
|---|---|---|
| `OSS_BUCKET` | 비어 있음 | Bucket 이름. 설정하면 OSS가 활성화되고 S3보다 우선함 |
| `OSS_REGION` | `cn-hangzhou` | OSS region. 예: `cn-hangzhou` |
| `OSS_ENDPOINT` | 비어 있음 | 선택적 OSS endpoint. 예: `https://oss-cn-hangzhou.aliyuncs.com` |
| `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET` | 비어 있음 | 정적 OSS 자격 증명 |
| `OSS_SECURITY_TOKEN` | 비어 있음 | 선택적 STS security token |
| `OSS_PUBLIC_BASE_URL` | 비어 있음 | 첨부 URL에 사용할 CDN 또는 사용자 지정 도메인 |
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `MULTICA_RELEASE_REPOSITORY` | 현재 CI 저장소 | Web 다운로드, CLI 업데이트, Desktop 패키징에서 릴리스 자산을 조회할 `owner/repo` |
...
Feishu Base automation 같은 외부 automation이 `POST /api/webhooks/external-issues`로 Multica issue를 만들 때 설정합니다.

| 변수 | 기본값 | 설명 |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | 비어 있음 | 외부 issue import 엔드포인트에 필요한 공유 secret. 요청은 `Authorization: Bearer <token>`을 보내야 합니다. 설정하지 않으면 엔드포인트가 `503 external issue import is not configured`를 반환합니다 |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_WORKSPACE_ID` | 비어 있음 | automation payload가 `workspace_id`를 생략할 때 사용할 기본 workspace id. Feishu Base automation은 보통 `app_token`, `table_id`, `record_id`만 보내므로 이 flow에서는 설정하세요 |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_LARK_USER_ID` | 비어 있음 | 기본 Multica member assignee / creator로 사용할 Feishu/Lark `user_id`. 해당 Lark 계정은 대상 workspace의 Multica 사용자에 이미 연결되어 있어야 합니다 |
| `MULTICA_EXTERNAL_ISSUE_LARK_OPENAPI_TIMEOUT` | `30s` | Feishu Base record 조회 timeout. `45s` 같은 Go duration 형식을 사용합니다 |

Feishu Base automation의 HTTP step Params는 다음처럼 설정할 수 있습니다.

| Param | 값 |
|---|---|
| `app_token` | Feishu automation의 Base app token |
| `table_id` | Feishu automation의 table id |
| `record_id` | Feishu automation의 record id |

Multica는 Lark login app의 `LARK_APP_ID`와 `LARK_APP_SECRET`로 해당 Base record를 가져오고, `版本类型`이 `小需求`인 record만 import합니다. issue description에는 `需求名称`, `产品经理`, `备注`, source URL이 포함됩니다. 첨부는 Feishu Base attachment field의 `file_token`, `name`, `size`, `type`, `url`, `tmp_url`에서 읽어 issue attachment로 업로드합니다.

가져온 issue는 `backlog`에 생성되며, `origin_type=external_issue`로 멱등 처리됩니다. 같은 외부 record를 retry하면 중복 생성하지 않고 기존 issue를 반환합니다.

## 다음 단계

- [로그인과 가입](/auth-setup) — 로그인 방식과 가입 제한을 알아봅니다.
- [GitHub 연동](/github-integration) — 연동에 사용할 GitHub App을 설정합니다.
