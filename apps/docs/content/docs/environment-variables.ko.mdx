...
Multica는 Lark login app의 `LARK_APP_ID`와 `LARK_APP_SECRET`로 해당 Base record를 가져오고, `版本类型`이 `小需求`인 record만 import합니다. issue description에는 `需求名称`, `产品经理`(person field의 이름), `备注`, source URL이 포함됩니다. 첨부는 Feishu Base attachment field의 `file_token`, `name`, `size`, `type`, `url`, `tmp_url`에서 읽어 issue attachment로 업로드합니다.
