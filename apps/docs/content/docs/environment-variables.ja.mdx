...
Multica は Lark login app の `LARK_APP_ID` と `LARK_APP_SECRET` を使って該当 Base record を取得し、`版本类型` が `小需求` の record だけを import します。添付は Feishu Base attachment field の `file_token`、`name`、`size`、`type`、`url`、`tmp_url` から読み取り、issue attachment としてアップロードします。
