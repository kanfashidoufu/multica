...
`OSS_BUCKET` が設定されている場合は Alibaba Cloud OSS、次に `S3_BUCKET` が設定されている場合は S3、それ以外はローカルディスクを使います。

### Alibaba Cloud OSS

| 変数 | デフォルト | 説明 |
|---|---|---|
| `OSS_BUCKET` | 空 | バケット名。設定すると OSS が有効になり、S3 より優先されます |
| `OSS_REGION` | `cn-hangzhou` | OSS リージョン。例: `cn-hangzhou` |
| `OSS_ENDPOINT` | 空 | 任意の OSS endpoint。例: `https://oss-cn-hangzhou.aliyuncs.com` |
| `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET` | 空 | 静的 OSS 資格情報 |
| `OSS_SECURITY_TOKEN` | 空 | 任意の STS security token |
| `OSS_PUBLIC_BASE_URL` | 空 | 添付 URL に使う CDN またはカスタムドメイン |
...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `MULTICA_RELEASE_REPOSITORY` | 現在の CI リポジトリ | Web ダウンロード、CLI 更新、Desktop パッケージがリリース資産を取得する `owner/repo` |
...
Feishu Base automation などの外部 automation が `POST /api/webhooks/external-issues` で Multica issue を作成する場合に設定します。

| 変数 | デフォルト | 説明 |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | 空 | 外部 issue import エンドポイントで必須の共有 secret。リクエストは `Authorization: Bearer <token>` を送信する必要があります。未設定の場合、エンドポイントは `503 external issue import is not configured` を返します |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_WORKSPACE_ID` | 空 | automation payload が `workspace_id` を省略した場合に使うデフォルト workspace id。Feishu Base automation は通常 `app_token`、`table_id`、`record_id` だけを送るため、この flow では設定してください |
| `MULTICA_EXTERNAL_ISSUE_DEFAULT_LARK_USER_ID` | 空 | デフォルトの Multica member assignee / creator として使う Feishu/Lark `user_id`。該当する Lark アカウントは、対象 workspace の Multica ユーザーに既にリンクされている必要があります |
| `MULTICA_EXTERNAL_ISSUE_LARK_OPENAPI_TIMEOUT` | `30s` | Feishu Base record を取得するときの timeout。`45s` などの Go duration を使用します |

Feishu Base automation の HTTP step Params は次のように設定できます。

| Param | 値 |
|---|---|
| `app_token` | Feishu automation の Base app token |
| `table_id` | Feishu automation の table id |
| `record_id` | Feishu automation の record id |

Multica は Lark login app の `LARK_APP_ID` と `LARK_APP_SECRET` を使って該当 Base record を取得し、`版本类型` が `小需求` の record だけを import します。issue description には `需求名称`、`产品经理`、`备注`、source URL が入ります。添付は Feishu Base attachment field の `file_token`、`name`、`size`、`type`、`url`、`tmp_url` から読み取り、issue attachment としてアップロードします。

インポートされた issue は `backlog` に作成され、`origin_type=external_issue` で冪等化されます。同じ外部 record の retry は重複作成せず、既存 issue を返します。

## 次のステップ

- [ログインとサインアップ](/auth-setup) — ログイン方式とサインアップ制限。
- [GitHub 連携](/github-integration) — 連携で使う GitHub App を設定します。
