...
Syndra が `POST /api/webhooks/external-issues` に Bug または小規模要件イベントを送信する場合に設定します。

| 変数 | デフォルト | 説明 |
|---|---|---|
| `MULTICA_EXTERNAL_ISSUE_WEBHOOK_TOKEN` | 空 | 同期エンドポイントで必須の共有 secret。リクエストは `Authorization: Bearer <token>` を送信する必要があります。未設定の場合、エンドポイントは `503 external issue sync is not configured` を返します |
| `MULTICA_EXTERNAL_BUG_WORKSPACE_ID` | 空 | `sync_type=bug` 専用の workspace id。リクエストからは上書きできません |
| `MULTICA_EXTERNAL_REQUIREMENT_WORKSPACE_ID` | 空 | `sync_type=requirement` 専用の workspace id。リクエストからは上書きできず、要件と Bug の workspace を分離します |

既存の Syndra Bug upsert フローには `sync_type=bug` を使用します。Bug 同期は送信された担当者名で workspace member を検索し、名前がない場合や一意に一致しない場合は Bug workspace の owner（workspace 作成者）にフォールバックします。小規模要件同期は `sync_type=requirement` を使用し、`executor_id` の Multica Agent を割り当て、その Agent の owner を issue creator として記録します。旧デフォルト workspace/assignee 変数と Feishu Base パラメータはサポートされません。
